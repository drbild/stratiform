...
# Parameters
deployment = parameter(name="Deployment",
...
domain_name = parameter(name="DomainName",
...
tags_public_named = tags_public + tag(Name=join('-', [STACK_NAME, 'public']))

# Resources
dhcp_options     = ec2.dhcp_options("DHCPOptions", tags_base + tag(Name=STACK_NAME), domain_name=domain_name)
vpc              = ec2.vpc_with_dns("VPC", cidr_vpc, dhcp_options, tags_base + tag(Name=STACK_NAME))
...
# Outputs
vpc_id                = output("VpcId", value=vpc, description="Id of the VPC")
public_subnet_id      = output("PublicSubnetId", value=public_subnet)
public_route_table_id = output("PublicRouteTableId", value=public_route_table)

# Create template
print template("VPC stack for bigsky docker registry").add_all(globals().itervalues())
